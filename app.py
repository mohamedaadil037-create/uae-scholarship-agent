...
# 3. API KEYS & SUPABASE INITIALIZATION
...
if "SUPABASE_URL" in st.secrets and "SUPABASE_KEY" in st.secrets:
    try:
        ...
        st.sidebar.warning("Vault connection pending...")

# Silent Database Logging Function
def log_to_vault(module_name, search_query, target_info):
    if supabase:
        try:
            ...
            pass # Fails silently so the app never crashes for the user
...
st.markdown('<div class="dev-footer"><span class="dev-icon">🛡️</span> Verified Builder: <b>Aadil</b></div>', unsafe_allow_html=True)
...
if mode == "🕵️‍♂️ Faculty Headhunter":
    ...
    if st.button("Initiate Scan 🔎", type="primary", use_container_width=True):
        log_to_vault("Faculty Headhunter", topic, uni) # SILENT DB TRACKING
        with st.status("🔄 Executing multi-hop search across faculty directories...", expanded=True):
            ...
            try:
                ...
                Task: List 3 high-value professor targets for '{topic}' at '{uni}'.
                Format:
                ### [Professor Name]
    ...
    with col2:
        if source_type == "ArXiv Database (Auto)":
            ...
            if st.button("📥 Fetch Data Stream", use_container_width=True):
                log_to_vault("Gap Finder (Auto)", paper_topic, "Tavily Stealth Mode") # SILENT DB TRACKING
            ...
            if uploaded_file:
                log_to_vault("Gap Finder (Manual)", uploaded_file.name, "PDF Upload") # SILENT DB TRACKING
...
# 8. APP LOGIC: MODE 3
    ...
    if st.session_state.chat_history[-1]["role"] == "user":
        log_to_vault("Defense Simulator", st.session_state.chat_history[-1]["content"], "MBZUAI Committee") # SILENT DB TRACKING
        with st.spinner("Committee is deliberating..."):
            ...
            Task: 1. Critique the answer sharply. 2. Ask a deeper, harder technical follow-up question.
            """
